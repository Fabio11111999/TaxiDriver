...
### Introduction
Taxi-driver is a simple project which uses Q-learning concepts to allow a taxi driver to learn the optimal way to pick up and drop off a passenger on different maps. To solve this kind of problems Q-learning isn't the most efficient strategy, there are, indeed, many path finding algorithms that guarantee much more efficient performances, the goal of this project is, in fact, to just show how a Q-learning approach can be used to solve it.
...
How you can see it's been trained for 150 episodes but it was able to get the optimal path much earlier than that. The few spikes are due to the fact that the approach has the possibility to make random choices, even though the probability of a random move is pretty low (1%) this may ruin the paths in some of the episodes.
